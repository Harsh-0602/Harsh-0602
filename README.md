<div align="center">

# Harsh Garg
**AI-first Product Builder | Student & Founder**  
Surat, Gujarat, India

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/harshgarg06/)
[![X](https://img.shields.io/badge/X-000000?style=flat&logo=x&logoColor=white)](https://x.com/HarshGarg002)

</div>

---

## About
I build **AI-first tools** and ship fast MVPs using **agentic workflows, automation, and integrations**.  
My goal: turn real problems into products, iterate quickly, and learn in public.

**Currently exploring**
- AI agents & workflow automation
- Prompt engineering + evaluation
- API integrations (webhooks, third‑party services)
- MVP building & product iteration

---

## Featured projects
| Project | Summary | Link |
|---|---|---|
| 📧 **MAP-001-AI-Email-Auto-Reply** | AI automation that generates and sends professional email replies using n8n + Gemini. | [Repo](https://github.com/Harsh-0602/MAP-001-AI-Email-Auto-Reply) |
| 🎯 **MAP-002-AI-Lead-Qualification** | AI workflow that scores and classifies incoming leads automatically. | [Repo](https://github.com/Harsh-0602/MAP-002-AI-Lead-Qualification) |
| 🔍 **resume-intelligence-ai** | AI explains why resumes get rejected + suggests improvements for ATS compatibility. | [Repo](https://github.com/Harsh-0602/resume-intelligence-ai) |
| 🤖 **SupportBridge** | AI support automation for SaaS teams (human‑in‑the‑loop). Currently onboarding early customers. | [Website](https://supportbridge.tech) |
| 📊 **commitscope** | Lightweight GitHub commit analytics & experiments. | [Repo](https://github.com/Harsh-0602/commitscope) |
| 🌸 **mindbloom-mood-tracker** | Mood tracking app with simple insights. | [Repo](https://github.com/Harsh-0602/mindbloom-mood-tracker) |

---

## Skills
**AI / Automation**
- AI agents (agentic workflows), prompt engineering, LLM tools
- Workflow automation, integrations, APIs/webhooks

**Dev (AI‑assisted prototyping)**
- JavaScript, Python, HTML/CSS
- Node.js, React (for prototypes)

**Tools**
- Git & GitHub, Google Cloud, VS Code

---

## Certifications
<div>
  <a href="https://drive.google.com/file/d/1BbCGv-KoAyZHtNkJHYQaDr9V-D0OuKay/view?usp=drive_link" target="_blank">
    <img alt="Google Prompting Essentials" src="https://img.shields.io/badge/Google%20Prompting%20Essentials-4285F4?style=flat&logo=google&logoColor=white" />
  </a>
  <a href="https://drive.google.com/file/d/1YSH7AjltPbejV_lSPwp3P74PkiVWiWxS/view?usp=drive_link" target="_blank">
    <img alt="Google AI Essentials" src="https://img.shields.io/badge/Google%20AI%20Essentials-34A853?style=flat&logo=google&logoColor=white" />
  </a>
</div>

---

## Contact
- LinkedIn: https://www.linkedin.com/in/harshgarg06/  
- X: https://x.com/HarshGarg002
